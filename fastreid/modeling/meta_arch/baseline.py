...
class Baseline(nn.Module):
    ...
    def losses(self, outputs, gt_labels):
        ...
        if 'MaxCone' in loss_names:
            ...
            loss, orth_measure = maxcone_loss(pred_features, gt_labels, temp, epochs = storage.epoch, all_epochs = maxcone_kwargs.get('all_epochs'), loss_type = maxcone_kwargs.get('loss_type') )
            #print(maxcone_kwargs.get('scale'))
        ...
        if 'SupConLoss' in loss_names:
            ...
            loss_dict['loss_maxcone'] = loss_func(nn.functional.normalize(pred_features, dim = 1).unsqueeze(1), gt_labels)*supcon_kwargs.get('scale')
        ...
        if 'MSLoss' in loss_names:
            ...
            loss_dict['loss_maxcone'] = loss_func(nn.functional.normalize(pred_features, dim = 1), gt_labels)*ms_kwargs.get('scale')
