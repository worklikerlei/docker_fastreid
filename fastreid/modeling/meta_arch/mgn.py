...
#from fastreid.modeling.losses import maxcone_loss, SupConLoss, MultiSimilarityLoss
...
class MGN(nn.Module):
            ...
            'loss_kwargs':
                {
                    ...
                    'maxcone': {
                        ...
                        'temp': cfg.MODEL.LOSSES.MAXCONE.TEMP,
                        'loss_type': cfg.MODEL.LOSSES.MAXCONE.TYPE,
    ...
    def losses(self,
        ...
        if "MaxCone" in loss_names:
            ...
            temp = maxcone_kwargs.get('temp')
            storage = get_event_storage()
            all_ep = maxcone_kwargs.get('all_epochs')
            loss_tp = maxcone_kwargs.get('loss_type')
            loss, orth_measure = maxcone_loss(b1_pool_feat, gt_labels, temp, epochs = storage.epoch, all_epochs = all_ep, loss_type = loss_tp)
            loss_dict['loss_maxcone_b1'] = loss*maxcone_kwargs.get('scale')* 0.2

            loss, orth_measure = maxcone_loss(b2_pool_feat, gt_labels, temp, epochs = storage.epoch, all_epochs = all_ep, loss_type = loss_tp)
            loss_dict['loss_maxcone_b2'] = loss*maxcone_kwargs.get('scale')* 0.2

            loss, orth_measure = maxcone_loss(b3_pool_feat, gt_labels, temp, epochs = storage.epoch, all_epochs = all_ep, loss_type = loss_tp)
            loss_dict['loss_maxcone_b3'] = loss*maxcone_kwargs.get('scale')* 0.2

            #loss, orth_measure = maxcone_loss(b21_pool_feat, gt_labels, temp, epochs = storage.epoch, all_epochs = all_ep, loss_type = loss_tp)
            #loss_dict['loss_maxcone_b21'] = loss*maxcone_kwargs.get('scale')* 0.125
            
            loss, orth_measure = maxcone_loss(b22_pool_feat, gt_labels, temp, epochs = storage.epoch, all_epochs = all_ep, loss_type = loss_tp)
            loss_dict['loss_maxcone_b22'] = loss*maxcone_kwargs.get('scale')* 0.2

            #loss, orth_measure = maxcone_loss(b31_pool_feat, gt_labels, temp, epochs = storage.epoch, all_epochs = all_ep, loss_type = loss_tp)
            #loss_dict['loss_maxcone_b31'] = loss*maxcone_kwargs.get('scale')* 0.125
            
            #loss, orth_measure = maxcone_loss(b32_pool_feat, gt_labels, temp, epochs = storage.epoch, all_epochs = all_ep, loss_type = loss_tp)
            #loss_dict['loss_maxcone_b32'] = loss*maxcone_kwargs.get('scale')* 0.125
            
            loss, orth_measure = maxcone_loss(b33_pool_feat, gt_labels, temp, epochs = storage.epoch, all_epochs = all_ep, loss_type = loss_tp)
            loss_dict['loss_maxcone_b33'] = loss*maxcone_kwargs.get('scale')* 0.2
        else:
            orth_measure = -1
