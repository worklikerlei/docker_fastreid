...
class ResNet(nn.Module):
    def __init__(self, last_stride, bn_norm, with_ibn, with_se, with_nl, block, layers, non_layers,mixstyle_layers=[], mixstyle_p=0.5, mixstyle_alpha=0.1,):
        self.inplanes = 64
        super().__init__()
...
def build_resnet_ms_backbone(cfg):
    ...
    mixstyle_layers = cfg.MODEL.BACKBONE.MIXSTYLE
    # fmt: on
    ...
    model = ResNet(last_stride, bn_norm, with_ibn, with_se, with_nl, block,
                   num_blocks_per_stage, nl_layers_per_stage,mixstyle_layers)
    if pretrain:
        print('+--------------------------------------------------------------------------------------------+')
